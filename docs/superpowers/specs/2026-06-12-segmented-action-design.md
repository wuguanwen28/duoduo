...
interface SegmentedActionDef {
  ...
  baseYoyo?: boolean;            // base 是否 yoyo 来回，默认 true
...
底层只依赖现有 `useSpriteAnimation`，通过给它喂「预切好 / yoyo 拼接 / 倒序」的帧数组来编排。**各段数组在 `start()` 时按配置预切一次并缓存**，不在每次插播时现切。
...
base     ：循环播放基底（yoyo 来回）；同时挂一个随机定时器，到点 → twitch
...
`yoyo = [...slice(s,e), ...slice(s,e).reverse()]`，再以 `loop:true` 交给播放器。`baseYoyo:false` 时退化为普通正向循环（idle 退化情形可用）。

**插播小动作同样支持 yoyo**：`TwitchDef.yoyo:true` 时，该 twitch 用同样的「正放+反放拼接」数组**正放一次**（非 loop），实现「甩出去再收回」的连续两下效果（如摇尾巴）。基底 yoyo 与插播 yoyo 复用同一个拼接辅助函数。
...
- **接缝跳帧**：base 用 yoyo 来回避免；ear/tail 起止帧与 base 端点不完全衔接会有轻微跳帧，属「大概区间」的已知代价，后续微调。
