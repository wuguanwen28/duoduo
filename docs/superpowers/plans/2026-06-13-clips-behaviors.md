...
export const CLIPS: Record<string, Clip> = {
  ...
  wink: { src: "wink", range: [0, FRAMES.wink.length], fps: 24, label: "wink" },
};
```
...
Expected: 零错误。若报 `FRAMES.wink` 不存在，确认 `src/actions/frames.ts` 的 `FRAMES` 含 `wink` 键（现状：`FRAMES = { wink, sleep }`）。
 ...
 *   follow  : 跟踪光标(注视)。
 *   action  : 播放一个被触发的行为(wink、sleep 等)。
 *
 * 状态转换
...
export function useCatBrain(opts: BrainOptions): CatBrain {
  ...
  async function tick() {
    ...
    switch (state.value.kind) {
      case "action": {
        // 仅 interruptible 显式为 true 的行为(wink)在鼠标移动时切回 follow。
...
改为：
```ts
/** 动作帧来源 → 有序的帧 URL。键由 `./clips.ts` 的 `SOURCES` 引用（如 sleep、wink）。 */
```
...
7. 普通 **idle** 行为与改动前一致(cat-idla 整段循环)；若 idle 自动选中 **wink** 则正常一次性播放并结束。
...
- 干掉双注册表、sleep 一条、wink 统一进来、`interruptible` 默认 false → Task 2 + Task 4 ✅
