...
console.log(
  `✓ 已下载并校验：duoduo.exe + version.json（sha256=${actual.slice(0, 12)}…）`,
)
...
if (giteeToken) {
  ...
  if (!rel || !rel.id) {
    console.warn(
      `⚠ 未取得 Gitee release id（token 失效或 release 创建失败？）：${JSON.stringify(rel).slice(0, 120)}`,
    )
  } else {
...
if (scpTarget) {
  ...
  execFileSync('ssh', [host, `mkdir -p ${path}/v${version}`], {
    stdio: 'inherit',
  })
  execFileSync('scp', [manifest, `${scpTarget}/version.json`], {
    stdio: 'inherit',
  })
  execFileSync('scp', [exe, `${scpTarget}/v${version}/duoduo.exe`], {
    stdio: 'inherit',
  })
} else {
