...
/** 默认说话模板跨窗口同步事件名。 */
export const SPEAK_PHRASES_DEFAULT_CHANGED_EVENT =
  'speak-phrases-default-changed'
...
export const speakPhrases = ref<SpeakPhrase[]>(
...
export function pickFromPool(pool: SpeakPhrase[]): string {
  const valid = (Array.isArray(pool) ? pool : []).filter(
    (p) => p && p.text && p.weight > 0,
  )
