...
export function useCatBrain(opts: BrainOptions): CatBrain {
  ...
  const beh = useBehavior(
    opts.runBuiltinTwitch ? { onTwitch: opts.runBuiltinTwitch } : {},
  )

  const state = ref<CatState>(
    followOnly
      ? { kind: 'follow' }
      : { kind: 'behavior', behavior: defaultBehavior },
  )
  ...
  const currentSrc = computed(() =>
    activePlayer.value === 'gaze'
      ? gaze.currentSrc.value
      : beh.currentSrc.value,
  )
  ...
  function playCurrentBehaviorTwitch() {
    ...
    const name =
      state.value.kind === 'behavior' ? currentBehavior : defaultBehavior
  ...
  async function tick() {
    ...
    if (lastPos) {
      moved =
        Math.hypot(pos.x - lastPos.x, pos.y - lastPos.y) > config.moveThreshold
    }
    lastPos = pos
  ...
  function nextDelay(): number {
    ...
    if (
      state.value.kind === 'behavior' &&
      behaviors[currentBehavior]?.interruptible !== true
    ) {
      return config.idleTickMs
    }

    // follow 进行中 / 可抢占的 idle：光标近期在动 → 高频；久未动 → 低频探测。
    return Date.now() - lastMoveAt > config.stillnessMs
      ? config.idleTickMs
      : config.tickMs
  }
