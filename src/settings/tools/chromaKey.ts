...
function bgMask(data: Uint8ClampedArray, w: number, h: number, Z: RegionRT[]): Uint8Array {
...
function minmax3(src: Uint8Array, w: number, h: number, erode: boolean): Uint8Array {
 ...
 */
export function autoDetectKeyColors(data: Uint8ClampedArray, w: number, h: number): KeyColor[] {
  ...
  for (const [cx, cy] of corners) {
    ...
    if (n === 0) continue;
    const c = { r: Math.round(r / n), g: Math.round(g / n), b: Math.round(b / n) };
