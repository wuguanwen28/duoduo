...
**发布流程**：
1. CI（或手动 `pnpm app:build`）产出 exe + `version.json`（含 sha256）。
2. `pnpm publish:mirrors` 将产物推到 Gitee Release / GitHub Release / 自建服务器。

**待填参数**（正式发布前须配置）：
- `SERVER_BASE`（`updater.rs`）：自建下载服务器域名。
- `GITEE_TOKEN`：Gitee Release 上传用的 Personal Access Token（CI 环境变量）。
- `DUODUO_SERVER_SCP`：自建服务器上传目标路径（CI 环境变量 / 脚本参数）。
